...
#run in cluster
snakemake --cluster "qsub -N test_snake -W group_list=bhurwitz -q {cluster.queue} -l {cluster.walltime} -l {cluster.select}" --cluster-config config/cluster.yaml -j 200 --latency-wait 15 --until virfinder
